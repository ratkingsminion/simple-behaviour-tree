...
```C#
    ...
    void Awake() {
      var targetPosIdx = -1;
      var targetPoss = new[] { Vector3.up, Vector3.right, Vector3.down, Vector3.left };
        ...
        .Parallel()
          ...
          .Repeat().Sequence("sequence walk around")
            .Do("walk to target pos",
              go => targetPos = go.transform.position + targetPoss[targetPosIdx = (targetPosIdx + 1) % 4],
              go => {
    ...
    void Colorize(GameObject go, Color color) {
      Debug.Log(Time.frameCount + "/" + Time.time + " <color=#" + ColorUtility.ToHtmlStringRGB(color) + ">ColorTest</color> " + name, go);
